...
There are other live coding tools for the web — [Strudel](https://strudel.cc/) in particular is excellent and worth checking out. Drome takes a different set of tradeoffs.

## Quantization

In Drome, your code changes audibly take effect at the **start of the next bar**. When you evaluate mid-bar, Drome processes your code immediately — so that it can load new samples and preemptively complete other setup work — but waits until the next bar before applying your changes.
