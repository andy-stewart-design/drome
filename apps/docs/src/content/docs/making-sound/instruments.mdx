...
| --------- | ---------------------------------- | -------- |
| ...cycles | `number \| number[] \| number[][]` | `true`   |

```js
.note(60, 64)              // two bars, one note each
.note([60, 64])            // one bar, two notes
.note(60, 64, 67, 71)      // four bars, one note each
.note([60, 64], [67, 71])  // two bars, two notes each
...
```js
.note("60, 62, 64")  // equivalent to .note(60, 62, 64)
```
