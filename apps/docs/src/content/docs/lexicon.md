...
A reference for the key terms and concepts you'll encounter when working with Drome.
...
- `fit` (default) — attack and decay fit within the note duration; release extends after it ends
- `clip` — all stages are proportionally scaled to fit entirely within the note duration
- `free` — stages are treated as absolute durations in seconds, independent of the pattern's timing
